...
router.post('/post_login',
    passport.authenticate('local', { failureRedirect: '/login?fail=true' }), function (req, res) {
    res.redirect('/select')
});

router.post('/post_invite',
    passport.authenticate('local', { failureRedirect: '/invite?fail=true' }), function (req, res) {
        let input = req.body;
        console.log(input);
        sql = `INSERT INTO Entrant(WorldID, id, permission) VALUES('${input.world}', '${input.id}', 2)`;
        try{
