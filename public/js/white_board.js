...
// @Public
    joinRoom(){
        ...
        // ホワイトボードの履歴を取得
        socket.emit('whiteboard-get', roomIDg);
    }
...
// クリックイベント
    ...
    _click_tool_reset = () => {
        ...
        if(!Result) return;
        socket.emit('whiteboard-reset', { roomID: this.status.room_id });
    }
    ...
    _click_whiteboard_btn = () => {
        ...
        if ($("#WhiteBoardArea").hasClass("hidden")){
            $("#WhiteBoardButton").attr("fill", "#ff8a85");
        }else{
